...
public class CreateRaceScreen extends JFrame {
	...
	public CreateRaceScreen(JFrame previousScreen) {
		...
		JLabel id = new JLabel("Informe o identificador da corrida:");
		id.setToolTipText("identificador da corrida");
		id.setFont(new Font("Arial", Font.BOLD, 18));
		JTextField idInput = new JTextField(30);
		idInput.setToolTipText("EX:1,2,ex1..");
...
//				dateInput.setText("19/05/2025 13:00");
				...
				if (id.isEmpty() || name.isEmpty() || city.isEmpty() || startFinishLocal.isEmpty() || distance.isEmpty()
						|| date.isEmpty() || timeLimitStr.isEmpty()) {
				...
				if (minutes < 0 || minutes > 59) {
					JOptionPane.showMessageDialog(null,
							"Minutos inválidos! O limite de minutos deve ser entre 0 e 59.");
					return;
				}
				...
				if (!DateValidator.validateDateToRace(date, Race.getFORMAT())) {
					JOptionPane.showMessageDialog(null,
							"A data não pode ja ter passado ou/e a data tem estar no formato dd/MM/yyyy HH:mm ");
					return;
				}

				if (!startFinishLocal.matches("^(?=.*[a-zA-Z])[^/]+/[^/]+$")) {
					JOptionPane.showMessageDialog(null,
							"O local de início e fim deve ser informado no formato 'localInicial/localFinal', e não pode conter apenas números.");
					return;
				}

				Race race = new Race(id, name, city, startFinishLocal, distance, date, limitRaceTime);

				if (race.getSucesso() != -1) {
